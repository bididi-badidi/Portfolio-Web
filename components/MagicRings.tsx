...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    const animate = (t: number) => {
      ...
      const scaledElapsed = elapsed * p.speed;
      const isStatic = p.playOnce && scaledElapsed >= p.staticAt;

      if (isStatic) {
        mouseRef.current[0] = 0;
        mouseRef.current[1] = 0;
        smoothMouseRef.current[0] = 0;
        smoothMouseRef.current[1] = 0;
      ...
      uniforms.uTime.value = p.playOnce ? Math.min(scaledElapsed, p.staticAt) : t * 0.001 * p.speed;
      ...
      uniforms.uMouseInfluence.value = !isStatic && p.followMouse ? p.mouseInfluence : 0;
      ...
      uniforms.uBurst.value = !isStatic && p.clickBurst ? burstRef.current : 0;
